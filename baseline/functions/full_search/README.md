...
这个目录是“云函数全量 HNSW 搜索”baseline。当前版本使用 Faiss `IndexHNSWFlat`，函数端加载：

```text
FAASANN_DATA_ROOT=/mnt/faasann
/mnt/faasann/sift100w/index/full/faiss_hnswflat.index
```

接口输入 `query`、`candidate_k`、`ef_search`，返回 `candidates`。这里的 `candidate_k` 是返回候选数量，`ef_search` 是 Faiss HNSW 查询深度。
...
- `ef_search`：Faiss HNSW 查询深度，越大通常 recall 越高、查询越慢。
...
| `avg_function_ann_search_ms`    | cloud function 内部开始执行 ANN 候选搜索                | ANN 搜索返回候选结果                        | 只看 ANN 搜索核。full baseline 是 Faiss HNSWFlat `index.search`；Two-stage baseline 和 FaasANNS 是 Faiss HNSWPQ `index.search`。不包含函数 handler 总包装、HTTP、server rerank。                                                |
...
- `faiss_search`：Faiss `IndexHNSWFlat.search()` 的实际搜索耗时，包含本次请求传入 `ef_search` 后的 HNSW 查询。
...
- `index_file_size_bytes`：函数端加载的 `faiss_hnswflat.index` 文件大小，单位是字节。
...
- `index_type`：当前 full baseline 索引类型，正常为 `faiss_hnswflat`。
- `metric_type`：Faiss 索引中的 metric type，L2 通常为 `1`。
...
- `avg_function_ann_search_ms`：云函数 ANN 搜索 kernel 耗时；full baseline 对应 `faiss_search`。
