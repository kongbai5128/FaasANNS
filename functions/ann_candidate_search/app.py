...
def _configure_dataset_from_argv(argv: list[str]) -> str:
    """从启动参数或环境变量确定数据集，并写回环境变量供 index_loader 使用。"""
...
class CandidateSearchHTTPHandler(BaseHTTPRequestHandler):
    """本地/自定义运行时 HTTP 适配器，把 HTTP 请求转给候选召回 handler。"""
    ...
    def do_POST(self) -> None:
        """解析 JSON 请求体并执行候选召回或 function-entry 查询。"""
...
def main() -> None:
    ...
    print(f"ann_candidate_search loading index before listening, dataset={DATASET}", flush=True)
    warmup()
    print(f"ann_candidate_search index loaded: {index_status()}", flush=True)
    ...
    server = HighConcurrencyHTTPServer(
        ("0.0.0.0", port),
        CandidateSearchHTTPHandler,
        backlog=backlog,
        max_workers=max_workers,
    )
    print(
        "ann_candidate_search listening on "
        f"0.0.0.0:{port}, dataset={DATASET}, backlog={backlog}, max_workers={max_workers}",
        flush=True,
    )
    try:
